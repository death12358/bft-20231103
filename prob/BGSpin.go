...
func BGSpinCalc(RTP string, TotalBet int64, HitFishList int32) ([5]int64, [5]float64) {
	...
	if BonusPayType == -1 {
		for mediumPayFishIdx := 0; mediumPayFishIdx < len(MediumPayFish); mediumPayFishIdx++ {
			if HitFishList == MediumPayFish[mediumPayFishIdx] {
				BonusPayType = 3
				break
			}
		}
	...
	for fishIdx := 0; fishIdx < KBonusPayFish; fishIdx++ {
		...
		if HitFishList == BonusPayFish[fishIdx] {
			if HitFishList == FISHC05 {
				isBgSpin = 1
			} else if HitFishList == FISHC06 {
	...
	if BonusPayType == 2 || BonusPayType == 3 {
		{
			//log.Printf("inBG")
			var calcWeight [2]int64
